...
export function App({ eventBus }: AppProps) {
  ...
  useEventBus(eventBus, GameEventNames.UI_SceneChange, (data) => {
  ...
  useEventBus(eventBus, GameEventNames.UI_StartGame, () => {
    console.log("Game starting, hiding React UI");
    setCurrentScene("GameScene");
  });

  const isInGame =
    currentScene === "GameScene" || currentScene === "SandboxScene";

  return (
    <div
      className={`w-full h-full ${
        isInGame ? "pointer-events-none" : "pointer-events-auto"
      }`}
    >
      {currentScene === "MenuScene" && <MainMenu eventBus={eventBus} />}

      {currentScene === "SoundTestScene" && <SoundTest eventBus={eventBus} />}
