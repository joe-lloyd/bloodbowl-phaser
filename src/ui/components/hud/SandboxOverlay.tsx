...
export function SandboxOverlay({ eventBus }: SandboxOverlayProps) {
    ...
    return (
        <div className="absolute top-24 right-4 w-64 bg-bb-warm-paper border-2 border-bb-gold rounded-lg p-4 shadow-lg pointer-events-auto z-50">
            <h3 className="text-bb-blood-red font-bold font-heading mb-4 text-center border-b border-bb-divider pb-2">SANDBOX TOOLS</h3>
            <div className="flex flex-col gap-2">
                <p className="text-xs text-bb-muted-text text-center italic mb-2">Select a scenario to reset state</p>
                {SCENARIOS.map(s => (
                    <Button key={s.id} onClick={() => emit('ui:loadScenario', { scenarioId: s.id })}
                ...
                <Button onClick={() => emit('ui:sceneChange', { scene: 'MenuScene' })}
