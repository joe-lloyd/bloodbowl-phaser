...
export const SetupControls: React.FC<SetupControlsProps> = ({ eventBus }) => {
    ...
    useEventBus(eventBus, 'ui:hideSetupControls', () => {
        setIsVisible(false);
    });

    useEventBus(eventBus, 'ui:setupcomplete', (complete: boolean) => {
        setIsComplete(complete);
    });

    const handleAction = (action: string) => {
        eventBus.emit('ui:setupAction', { action });
    };
            ...
            <div className="flex flex-col gap-2 pointer-events-auto">
                ...
                <div className="mt-2">
                    <Button
                        ...
                        disabled={!isComplete}
                        className={`w-full shadow-lg ${isComplete ? 'animate-pulse' : 'opacity-50'}`}
                    >
                        CONFIRM SETUP
                    </Button>
                </div>
