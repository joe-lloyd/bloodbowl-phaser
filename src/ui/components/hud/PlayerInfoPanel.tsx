...
export const PlayerInfoPanel: React.FC<PlayerInfoPanelProps> = ({ eventBus }) => {
    const [selectedPlayer, setSelectedPlayer] = useState<Player | null>(null);
    const [hoveredPlayer, setHoveredPlayer] = useState<Player | null>(null);

    // Hover Events
    useEventBus(eventBus, 'ui:showPlayerInfo', (player: Player) => {
        setHoveredPlayer(player);
    });

    useEventBus(eventBus, 'ui:hidePlayerInfo', () => {
    ...
    useEventBus(eventBus, 'playerSelected', (data: { player: Player | null }) => {
                ...
                <div className={`bg-[#2a2a3e]/95 border-2 ${borderColor} rounded-lg p-3 shadow-lg text-white`}>
                    ...
                    <div className="mb-2 border-b border-gray-600 pb-2">
                        <div className={`text-lg font-bold ${titleColor} leading-tight`}>
                            {isComparison && <span className="text-xs block text-gray-400 mb-1">COMPARING</span>}
                    ...
                    {/* Skills */}
                    <div className="mb-1">
                        <span className="text-xs font-bold text-green-400 block mb-0.5">Skills</span>
    ...
    return (
        <>
            ...
            {hoveredPlayer && hoveredPlayer.id !== selectedPlayer?.id && renderPanel(hoveredPlayer, true)}
        </>
    );
};
