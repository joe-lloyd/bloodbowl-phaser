...
export class ActionValidator {
    ...
    private validateBlock(actor: Player, target: Player): ActionResult {
        // Must be adjacent
        const dx = Math.abs((actor.gridPosition?.x || 0) - (target.gridPosition?.x || 0));
        const dy = Math.abs((actor.gridPosition?.y || 0) - (target.gridPosition?.y || 0));
        ...
        if (target.status !== 'Active' && target.status !== 'Stunned') { // Simplified status check
    ...
    private validatePass(actor: Player, target: { x: number, y: number }): ActionResult {
