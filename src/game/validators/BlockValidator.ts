...
export class BlockValidator {
    ...
    public analyzeBlock(
        ...
        // 1. Calculate Assists
        const attackerAssists = this.getValidAssists(attacker, defender, allPlayers);
        const defenderAssists = this.getValidAssists(defender, attacker, allPlayers);
    ...
    public getValidAssists(
        ...
        const teammates = allPlayers.filter(p => p.teamId === subject.teamId && p.id !== subject.id);
    ...
    private isMarkedByOthers(player: Player, enemies: Player[], excludedEnemy: Player): boolean {
