...
import { initAudioOnFirstClick, getAudioContext, initStrudel } from '@strudel/web';
...
export class SoundManager {
    ...
    public async playOpeningTheme(): Promise<void> {
        ...
        if (ctx?.state === 'suspended') {
            console.warn("SoundManager: AudioContext is suspended! Waiting for resume...");
            try {
                await ctx.resume();
        ...
        if (this.currentCycle) {
            ...
            try {
                if (typeof this.currentCycle.stop === 'function') {
                    ...
                    console.warn("SoundManager: Cycle object has no stop/pause method!", this.currentCycle);
                }
            } catch (e) {
