 ...
 */
export type BlockResultType = 'both-down' | 'skull' | 'push' | 'pow' | 'pow-dodge';
...
export class BlockResolutionService {
     ...
     */
    public static selectBlockResult(results: BlockResult[], isAttackerChoice: boolean): BlockResult {
        // Priority order (best to worst for attacker)
        const priority: BlockResultType[] = ['pow', 'pow-dodge', 'push', 'both-down', 'skull'];
    ...
    public static getValidPushDirections(
        ...
        return directions.filter(pos =>
            pos.x >= 0 && pos.x < 26 && pos.y >= 0 && pos.y < 15
        );
    }
    ...
    public static allowsFollowUp(resultType: BlockResultType): boolean {
        return resultType === 'pow' || resultType === 'pow-dodge' || resultType === 'push';
    }
}
