...
export class GameService implements IGameService {
    ...
    constructor(
        ...
        this.ballManager = new BallManager(eventBus, this.state, team1, team2, this.weatherService, {
            onTurnover: (reason) => this.triggerTurnover(reason),
            onPhaseChange: (phase, subPhase) => this.eventBus.emit('phaseChanged', { phase, subPhase }),
            onBallPlaced: (x, y) => this.eventBus.emit('ballPlaced', { x, y })
        });

        this.movementManager = new MovementManager(eventBus, this.state, team1, team2, this.ballManager, {
            onTurnover: (reason) => this.triggerTurnover(reason),
    ...
    rollBlockDice(attackerId: string, defenderId: string, numDice: number, isAttackerChoice: boolean): void {
        this.blockManager.rollBlockDice(attackerId, defenderId, numDice, isAttackerChoice);
    }
    ...
    resetForKickoff(): void {
        ...
        if (scoringTeamId) {
            ...
            // Fallback
            this.startSetup(this.turnManager.getDriveKickingTeamId() || this.team1.id);
        }
    }
