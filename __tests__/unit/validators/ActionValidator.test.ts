...
import { ActionValidator } from '../../../src/domain/validators/ActionValidator';
import { PlayerBuilder } from '../../utils/test-builders';

describe('ActionValidator', () => {
    let validator: ActionValidator;
    const attacker = new PlayerBuilder().withId('atk').withPosition(5, 5).build();
    const defender = new PlayerBuilder().withId('def').withPosition(6, 5).build();
    ...
    describe('Pass Action', () => {
        it('should allow pass (basic check)', () => {
            const result = validator.validateAction('pass', attacker, { x: 10, y: 10 });
            expect(result.valid).toBe(true);
        });
    });
});
