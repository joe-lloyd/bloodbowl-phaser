...
describe('Kickoff Verification', () => {
    ...
    beforeEach(() => {
        ...
        // Setup Team 1
        for (let i = 0; i < 7; i++) gameService.placePlayer(team1.players[i].id, 0, 5 + i > 10 ? 10 : 5 + i); // Just valid spots
        gameService.confirmSetup('t1');

        // Setup Team 2
        for (let i = 0; i < 7; i++) gameService.placePlayer(team2.players[i].id, 14, 5 + i > 10 ? 10 : 5 + i);
    ...
    it('should roll 2D6 (2-12) for kickoff result', () => new Promise<void>(done => {
    ...
    it('should emit specific events for specific rolls (mocked)', () => new Promise<void>(done => {
