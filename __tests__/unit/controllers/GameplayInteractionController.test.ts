...
describe('GameplayInteractionController', () => {
    ...
    const player1 = { id: 'p1', teamId: team1Id, gridPosition: { x: 5, y: 5 }, stats: { MA: 6 }, status: 'Active' };
    const player2 = { id: 'p2', teamId: team2Id, gridPosition: { x: 6, y: 6 }, stats: { MA: 6 }, status: 'Active' };

    beforeEach(() => {
        vi.clearAllMocks();

        // Setup default scene mock for teams (as the controller accesses them directly in a temporary hack)
        (mockScene as any).team1 = { id: team1Id, players: [player1], colors: { primary: 0xff0000 } };
        (mockScene as any).team2 = { id: team2Id, players: [player2], colors: { primary: 0x0000ff } };
        ...
        controller = new GameplayInteractionController(
            ...
            mockPlayerInfoPanel as any
        );
    });

    describe('Player Selection', () => {
        it('should select player and show visuals if it is their turn', () => {
            mockGameService.getState.mockReturnValue({ activeTeamId: team1Id } as GameState);
        ...
        it('should select player but NOT show movement visuals if NOT their turn', () => {
            mockGameService.getState.mockReturnValue({ activeTeamId: team2Id } as GameState); // Team 2 is active
    ...
    describe('Movement Planning (Waypoints)', () => {
        ...
        it('should execute move when clicking the last waypoint', async () => {
            // 1. Add Waypoint
            mockMovementValidator.findPath.mockReturnValue({ valid: true, path: [{ x: 6, y: 5 }], rolls: [] });
            (controller as any).onSquareClicked(6, 5); // Add

            // 2. Click Same Spot (Confirm)
            (controller as any).onSquareClicked(6, 5); // Confirm

            expect(mockGameService.movePlayer).toHaveBeenCalledWith('p1', [{ x: 6, y: 5 }]);
