...
Define the online lobby: hosting and joining by match code, per-player team selection from one's own library, ready-up gating before start, one active match per player with resume, saving and resuming a match later, mutual end-of-match agreement, and host-controlled match settings.
