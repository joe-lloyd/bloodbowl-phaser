...
Ensure a Foul's KO, Casualty, and Send-Off outcomes relocate the affected player off the pitch (KO box, dead-and-injured, sent-off/dugout-with-marker) and reconcile the view, mirroring the existing block-caused injury seam — instead of mutating `player.status` directly and leaving the pitch sprite behind.
