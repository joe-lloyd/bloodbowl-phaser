...
Define the Blood Bowl Sevens kickoff table (2-12), the non-interactive events it can produce, the drive- and turn-scoped effects those events create, and how the roll and its resolution are made legible in the match log — independent of the interactive events that require coach input (see `kickoff-event-interactions`).
