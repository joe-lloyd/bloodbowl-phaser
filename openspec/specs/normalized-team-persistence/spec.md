...
Stored player and team records carry only non-derivable data — identity, progression, injuries, and career statistics — while statlines, resolved skills, keywords, skill-category access, cost, and team value are rehydrated from the roster templates on load. This is the single authoritative shape for a team crossing the persistence boundary (localStorage and Firestore), so a saved team never drifts from the rulebook data it derives from.
