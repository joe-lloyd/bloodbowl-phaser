...
Define how the three block-dice re-roll sources — Team Re-roll, Pro, and Brawler — surface to the coach: as buttons/commands on the existing `block-dice` pending decision rather than as separate confirmation prompts, with centrally enforced mutual exclusion and deterministic, observable outcomes drawn from the seeded dice service.
