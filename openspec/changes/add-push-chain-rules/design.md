...
**Deviation (implementation):** the chain lives as private state inside `BlockManager` (`chain` + `requestPushDecision` + `applyChain`) rather than a separate `PushResolver` class — the resolver would have needed the manager's event bus, teams, and flow manager anyway, so extracting it only added indirection. Semantics are exactly as designed (deferred application, innermost first, blocker chooses every link). Post-implementation additions the chain surfaced: the ball follows a pushed carrier (and bounces when a POWed carrier lands); a dropped bounce/throw-in is no longer a turnover (`CatchOperation.turnoverOnDrop`); a ball bouncing off-pitch is thrown in instead of voided; a defender surfed straight from the block ends the attacker's activation via a trailing `FinishActivationOperation` so the throw-in settles before any auto end-turn.
