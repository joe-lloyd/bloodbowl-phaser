...
- **Events that need a coach act before the ball lands.** Solid Defence, High Kick, Quick Snap and Charge! SHALL open an interactive step for the owning coach, but player choices and actions SHALL happen directly on the pitch through the game's existing selection, drag/setup placement, context-menu, and action-window interactions. Solid Defence SHALL let the coach drag each eligible player directly from its current pitch square to a legal new setup square; it SHALL NOT temporarily remove that player to Reserves or require a separate selection pass. The kickoff popup SHALL explain the event, occupy the bottom-right temporary-menu area, and expose only event-level controls such as confirm or skip; it SHALL NOT contain a second player-selection UI. The kickoff SHALL wait for every selected action to complete or be skipped before the receiving team's turn begins.
- **The visual kickoff follows one authoritative sequence.** After a kicker and target are selected, deviation SHALL move the single real ball into an enlarged, semi-transparent airborne state above the deviated square. The table SHALL then roll and fully resolve with the camera fixed in its existing gameplay view. Only afterward SHALL that same ball animate back to normal size and full opacity, handing off to the ordinary catch or bounce rules.
...
- **Results are legible.** The roll, the named event and its rulebook meaning SHALL be recorded in the match log. Contested rolls SHALL name the team beside each die and modifier, and every affected player or team SHALL be named with the resulting state change rather than receiving a silent effect.
...
- UI: a lightweight kickoff-event status step in `src/ui/components/hud/` (meaning, progress, confirm, skip) while all player selection and action execution reuse the pitch, setup drag flow, context menu, and existing action window.
