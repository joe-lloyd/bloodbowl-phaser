...
## Update: the roll-once guard had to cover `kickBall()`, not just the table roll

A first pass guarded only `KickoffEventManager.rollAndResolve()` (the 2D6 table roll) via `GameState.kickoffResolution`. A follow-up review traced `state.subPhase` end-to-end and found it stays `ROLL_KICKOFF` for the **entire** kickoff sequence — deviation, table roll, any interactive step (which can suspend indefinitely awaiting a coach), ball landing, placement — it only advances once `TurnManager.startGame()` sets `TURN_RECEIVING` at the very end. `RESOLVE_KICKOFF`/`PLACE_BALL` are only ever used as `PhaseChanged` event payloads for UI display, never persisted to `state.subPhase`. Since `GameplayInteractionController`'s click handler offers "Select Kicker & Target" whenever `subPhase === ROLL_KICKOFF` and unconditionally calls `gameService.kickBall()`, a stale re-entry anywhere in that (much wider than assumed) window could still re-roll the deviation and the table, landing the ball on a different square even with the table-only guard in place — reproduced with a headless test that saved mid-Charge!-step and called the `kick-ball` command a second time.

Fix: added a second, earlier guard field, `GameState.kickoffKickResolved`, set the moment `BallManager.kickBall()` computes deviation (before the table even rolls) and checked at the very top of `kickBall()`. When set, a re-entrant `kickBall()` call skips recomputing deviation and re-asking the "On the Ball" reaction entirely, and instead replays from `state.ballPosition` (already correctly restored from the snapshot) through the same `resolveKickoffEvent()` → `finishKickoffResolution()` path, which in turn hits the existing table-roll guard. Both fields are cleared together in `resetDriveState()`.
