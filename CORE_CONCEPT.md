# 🏈 **Blood Bowl — Core Concepts & Rules**
...
## 2. Turn Order (Player Turn)

You take **one full turn**, then opponent takes one turn, repeat.
...
Each player model can take **1 Action per turn**:

| Action       | What it does                                              |
| ------------ | --------------------------------------------------------- |
| **Move**     | Walk up to full MA (Movement Allowance)                   |
| **Block**    | Hit an adjacent opponent (no further moves unless Blitz)  |
| **Blitz**    | Move + 1 Block somewhere along the path                   |
| **Pass**     | Throw the ball to a target square/player                  |
| **Hand-Off** | Give ball to adjacent teammate (counts as pass action)    |
| **Foul**     | Kick a downed opponent (Risky, causes turnover if caught) |
...
1. **Kick-off**
2. **Ball scatters**
3. You play 8 turns to try score/stop score.
4. Reset for next drive until half/game ends.
...
| ------------------------------- | -------------------------------------------- |
| Lineman                         | 50–70k                                       |
| Blocker/Catcher/Thrower/Blitzer | 80–120k                                      |
| Team Re-Roll                    | 50–70k+ (more expensive after team creation) |

---

# ⭐ Leveling Up After a Game

## 1. Earn Star Player Points (SPP)

You earn XP for key plays:

| Event                    | SPP |
| ------------------------ | --- |
| Touchdown                | 3   |
| Casualty                 | 2   |
| Completion (pass caught) | 1   |
| MVP (random player)      | 4   |

League bonus SPP may apply.

---

## 2. Spend SPP for Levels
...
1. Roll for **injury recovery**
2. Award **MVP**
3. Add SPP and level players
4. Roll **winnings** (d6 + modifiers)
5. Buy new players, staff, or save cash
6. Update **Team Value (TV)**
...
# 7s — Blood Bowl Sevens

---

# 🏈 **Blood Bowl Sevens Rules**

## 1. Game Differences vs Standard

| Standard            | Sevens                    |
| ------------------- | ------------------------- |
| 11+ players         | **7 players**             |
| 8 turns per half    | **6 turns per half**      |
| 1,000,000 gold      | **600,000 gold**          |
| Big pitch           | **Reduced pitch (11x17)** |
| Normal skill access | **More random level ups** |
| Kick-off table used | **Simpler kick rules**    |
...
3. SPP
4. Level up
5. Winnings (usually smaller d6)
6. Update TV & buy stuff
